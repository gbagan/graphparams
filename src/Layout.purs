...
step _ layout = layout

center :: Number -> Number -> Array Position -> Array Position
center cx cy layout =
    let n = toNumber $ length layout
        sumx = foldl (+) 0.0 (_.x <$> layout)
        sumy = foldl (+) 0.0 (_.y <$> layout)
        sx = (sumx / n - cx)
        sy = (sumy / n - cy)
    in
    layout <#> \{x, y} -> {x: x - sx, y: y - sy}

force alpha edges layout =
    layout # mapWithIndex \i {x, y} ->
        let rforce =
                layout # mapWithIndex \j {x: x', y: y'} ->
                    if i == j then
                        {x: 0.0, y: 0.0}
                    else 
                        let distx = (x - x') * (x - x') 

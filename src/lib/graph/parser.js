import {addEdge, clean, graph} from './graph';
import {biclique, clique, cycle, grid, path, petersen, sun} from './generate';
...
const parse = str => {
    ...
    const methods = new Map([
        ...
        ['addClique',  {arity: -1, type: ParserToken.Number, fn: (g, ...args) => addClique(args, g)}],
        ['addPath',    {arity: -1, type: ParserToken.Number, fn: (g, ...args) => addPath(args, g)}],
        ['addCycle',   {arity: -1, type: ParserToken.Number, fn: (g, ...args) => addCycle(args, g)}],
        ['addEdges',   {arity: -1, type: ParserToken.Edge,   fn: (g, ...args) => addEdges(args, g)}],
        ...
        if (currentToken === LexerToken.Number) {
            ...
            if (!fntoken) {
                ...
                if (parameters.length !== method.arity && method.arity !== -1) {
                    ...
                    const fn = method.fn;
                    const res = fn(graphToken.data, ...parameters.map(x => x.data));
                    stack.push({ type: ParserToken.Graph, data: res || graphToken.data });
                }
            }
    ...
    return !token || token.type !== ParserToken.Graph ? 'invalid type' : clean(token.data);
}

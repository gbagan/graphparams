import {F, map, maxBy, sum, range, times} from '@fp';
import {binaryDecode, binaryEncode} from '../util';
import {complement} from './operators';

export const nbVertices = graph => graph.V;
export const nbEdges = graph => sum(graph.adj.map(nbor => nbor.length)) / 2;
export const minDegree = graph => Math.min(...graph.adj.map(nbor => nbor.length));
export const maxDegree = graph => Math.max(...graph.adj.map(nbor => nbor.length));
export const isRegular = graph  => minDegree(graph) === maxDegree(graph);

export const isConnected = graph => {
    const visited = times(F, graph.V);
    const stack = [];
    let nbVisited = 1;
    visited[0] = true;
    stack.push(0);
    ...
    return nbVisited === graph.V;
}

export const isHamiltonian = graph => graph.V === 1 && { result: true, witness: [0] }
                                        || graph.V === 2  && { result: false, witness: null }
                                        || hamiltonAux(graph, [0]);

const hamiltonAux = (graph, path) => {
    const last = path[path.length - 1];
    if (path.length === graph.V) {
        return graph.V === 1 || graph.hasEdge(path[0], last) ?
                    { result: true, witness: path } : { result: false, witness: null };
    } else {
        for (const v of graph.adj[last]) {
            if (path.includes(v)) {
                continue;
            }
...
export const degeneracy = graph => {
    const set = new Set(range(0, graph.V));
    const order = [];
    let maxdegree = 0;
    while (set.size > 0) {
        let minDeg = Infinity;
        let bestVertex = null;
        for (const v of set) {
            const degree = sum(map(u => set.has(u) ? 1: 0, graph.adj[v]));
            if (degree < minDeg) {
                bestVertex = v;
                minDeg = degree;
            }
...
export const eccentricity = (graph, vertex) => {
    const distance = times(_ => -1, graph.V);
    ...
    if (nbVisited !== graph.V) {
        return { result: Infinity, witness: null };
    } else {
        let u = maxBy(w => distance[w], range(0, graph.V));
        const path = [u];
        while (u !== vertex) {
            u = parent[u];
            path.push(u);
...
export const mis = graph => {
    let isets = [0];
    let i = 0;
    const nbors = graph.adj.map(adj => binaryEncode(adj));
